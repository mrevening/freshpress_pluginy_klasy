...
////        echo 'jestem tu';
////        return $this->db->get_var("SELECT COUNT(*) FROM `DaneDemograficzne`;");
//        $user_count = $this->db->get_var( "SELECT COUNT(*) FROM danedemograficzne" );
    ...
    public function create_lekarze_from_users ()
    {
        ...
        foreach ($dane as $row) {
            ...
            if (!email_exists($row->{'UserName'})) {
                ...
                $cos = wp_set_object_terms($user_created_new_id, $term, $taxonomy, true);
                var_dump($cos);
                echo ' ' . $rola;
            }
            echo "</BR>";
