...
{
    private $db;
    private $limit_users = ' ORDER BY PESEL ASC LIMIT 5';//' '; //WHERE PESEL LIKE 57112417752
    private $limit_patients = ' ORDER BY PESEL ASC LIMIT 5';
    private $limit_lekarze = '';
...
//                $this->add_user_metas_post($user_created_new_id, $lista, $user_dane);
    ...
    private function add_user_metas_post($post_id, $lista, $row, $tryb='single' ){
        if ($row == NULL) return;
        foreach ($lista as $kolumna) {
        switch ($tryb) {
            case 'single':
    ...
    private function insert_ankieta($tytul, $user_id, $row, $list){
        ...
        switch ($tytul) {
            ...
            case 'patients_eq5d':
                ...
                update_post_meta($id, 'rejsm_eq5d_wynik', $wynik );
